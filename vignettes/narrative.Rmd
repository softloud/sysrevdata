...
The objective of a <u class="dotted">`r tippy("narrative synthesis","<span style='font-size:15px;'><em>Narrative synthesis</em> refers to an approach to the systematic review and synthesis of findings from multiple studies that relies primarily on the use of words and text to summarise and explain the findings of the synthesis. Whilst narrative synthesis can involve the manipulation of statistical data, the defining characteristic is that it adopts a textual approach to the process of synthesis to ‘tell the story’ of the findings from the included studies.' <em>From: Popay, J., Roberts, H., Sowden, A., Petticrew, M., Arai, L., Rodgers, M., Britten, N., Roen, K. and Duffy, S. 2006. Guidance on the conduct of narrative synthesis in systematic reviews. A product from the ESRC methods programme Version, 1, p.b92.</span>")`</u> is to summarise each study included in a systematic review or systematic map, often using a <u class="dotted">`r tippy("human-readable", "<span style='font-size:15px;'>i.e. easy-to-read and comprehend, readily digestible format</span>")`</u> table:
...
Now we adapt the code we used to condense the variables to a <u class="dotted">`r tippy("function", "<span style='font-size:15px;'> Functions are mini R-based 'programmes' that allow us to replicate an action easily if we're likely to perform it more than once.</span>")`</u>. 
...
buffer_example_long %>% 
  ...
  select(short_title, category, subcategory_value)


```


Now we'll <u class="dotted">`r tippy("pivot", "<span style='font-size:15px;'><em>Pivoting</em> refers to changing the orientation of a dataset from wide to long or vice versa</span>")`</u> wide so we can condense. 
